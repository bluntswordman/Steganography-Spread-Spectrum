...
public class StringManipulation {
  public StringBuilder spreadingBits(StringBuilder binary) {
    ...
    for (int i = 0; i < binary.length(); i++) {
      spreadingBits.append(binary.charAt(i)).append(binary.charAt(i)).append(binary.charAt(i)).append(binary.charAt(i));
    }
  ...
  public StringBuilder deSpreadingBits(StringBuilder spreadingBits) {
    ...
    for (int i = 0; i < spreadingBits.length(); i += 4) {
      binary.append(spreadingBits.charAt(i));
    }
    return binary;
  }
  ...
  public int[] convertBinaryToBits(StringBuilder binary) {
    ...
    for (int i = 0; i < binary.length(); i++) {
      bits[i] = Integer.parseInt(String.valueOf(binary.charAt(i)));
    }
    return bits;
  }
